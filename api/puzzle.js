...
- Examples: go outside, call someone, notice something, write something down, try something different
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fetch used words from Firestore to avoid repeats
    let usedWords = []
    try {
      const db = await getAdminDb()
      const snap = await db.collection('dailyPuzzles')
...
Return ONLY raw JSON with no markdown:
{
  ...
  "world_note": null,
  "challenge": "Today do one specific gentle free action — NEVER suggest buying anything, spending money, or acquiring products",
  ...
  "synonyms": ["2-3 common synonyms players might guess"]
    ...
    // Save to word bank so it's never repeated
    try {
      const db = await getAdminDb()
