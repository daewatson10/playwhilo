...
function getWeekKeyET() {
  ...
  const diff = d.getDate() - day + (day === 0 ? -6 : 1)
   new Date(d.setDate(diff)).toLocaleDateString('en-CA')
}
...
export function useWhilo(user) {
  ...
  async function loadWeekTheme() {
    ...
    try {
      ...
      if (cached) setWeekTheme(JSON.parse(cached))
      else setWeekTheme({ theme_name: 'Reflection' })
    }
  }
  ...
  async function loadUserDay(uid, date) {
    try {
      ...
      if (snap.exists()) {
        const data = snap.data()
        // Save to localStorage under the correct key so archive and calendar can read it
  ...
  async function startPuzzle(date) {
    if (date < '2026-05-01') return null
    setActiveDate(date)
  ...
  function revealClue(date, idx, isSolved) {
    ...
    if (isSolved) {
      // Post-solve: just return updated state for display, don't save
      return { ...s, cluesUsed: [...used, idx] }
    }
  ...
  return {
    ...
    TODAY, load, localSave, cloudSave, loadUserDay,
